...
parser.add_argument('--fold', default=1, type=int, help='cv folder')
...
def main():
    ...
    label_id_path_file = 'train_30class_idx.txt'
    cv_test_fold_id = args.fold

    # for the base training of transfer learning
    Selected_cells_num = 5
    if not args.evaluate:

        full_trainset = CellDataset_CV(data_path=full_data_path
                               ,train=True,transform=transform_train,
                                       label_id_path_file=label_id_path_file,cv_test_fold_id=cv_test_fold_id,Selected_cells=Selected_cells_num)
        ...
        print('train class: ', train_class_num)
    else:
        testset = CellDataset(train_data_path=full_data_path,
                              train=False,transform=transform_test)
        testloader = torch.utils.data.DataLoader(testset, batch_size=args.bs, shuffle=False, num_workers=4)

        train_class_num = testset.class_number

    args.checkpoint = './checkpoints/DATA_WELL_CV/{}_base{}'.format(args.arch, Selected_cells_num)
    ...
    if 'ResNet' in args.arch:
        ...
        net = models.__dict__[args.arch](num_classes=train_class_num,backbone_fc=True,in_channel=3)
    ...
    # mobilenet_v2 inception_v3
        ...
        net = torch_model.__dict__[args.arch](pretrained=False, num_classes=train_class_num)
    ...
    if not args.evaluate:
        ...
        for epoch in range(start_epoch, start_epoch + args.es):
            ...
            adjust_learning_rate(optimizer, epoch, args.lr,step=25)
